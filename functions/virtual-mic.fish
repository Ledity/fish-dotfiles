function virtual-mic --description 'add a virtual microphone decieve apps'
    ...
    # EXISTS
    if test "$SERVER" = "pw"
        if pw-cli list-objects | grep "pw-virt-mic@$(whoami)" &> /dev/null
            set -f EXISTS "true"
        else
            set -f EXISTS "nil"
        end
    else if test "$SERVER" = "pa"
        if pactl list | grep "pa-virt-mic@$(whoami)" &> /dev/null
            set -f EXISTS "true"
        else
            set -f EXISTS "nil"
        end
